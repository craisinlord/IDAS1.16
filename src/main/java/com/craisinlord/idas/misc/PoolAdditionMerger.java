...
import com.craisinlord.idas.mixin.resources.FallbackResourceManagerAccessor;
import com.craisinlord.idas.mixin.resources.SimpleReloadableResourceManagerAccessor;
import com.craisinlord.idas.mixin.structures.JigsawPatternAccessor;
import com.craisinlord.idas.mixin.structures.TemplateManagerAccessor;
import com.craisinlord.idas.utils.SafeDecodingRegistryOps;
...
public final class PoolAdditionMerger {
    ...
    public static void mergeAdditionPools(final FMLServerAboutToStartEvent event) {
        IResourceManager resourceManager = ((TemplateManagerAccessor) event.getServer().getStructureManager()).idas_getResourceManager();
    ...
    private static Map<ResourceLocation, List<JsonElement>> getPoolAdditionJSON(IResourceManager resourceManager) {
        ...
        for (ResourceLocation fileIDWithExtension : resourceManager.listResources(DATA_TYPE, (fileString) -> fileString.endsWith(".json"))) {
            ...
            try {
                ...
                for (InputStream fileStream : getAllFileStreams(resourceManager, fileIDWithExtension)) {
                    try (Reader bufferedReader = new BufferedReader(new InputStreamReader(fileStream, StandardCharsets.UTF_8))) {
                        ...
                        if (poolJSONElement != null) {
                            ...
                            IDAS.LOGGER.error(
                                    "(Integrated Dungeons and Structures POOL MERGER) Couldn't load data file {} from {} as it's null or empty",
                                    fileID,
                                    fileIDWithExtension);
                        }
                    }
                ...
                IDAS.LOGGER.error(
                        "(Integrated Dungeons and Structures POOL MERGER) Couldn't parse data file {} from {}",
                        fileID,
                        fileIDWithExtension,
                        exception);
            }
    ...
    private static List<InputStream> getAllFileStreams(IResourceManager resourceManager, ResourceLocation fileID) throws IOException {
        ...
        FallbackResourceManager namespaceResourceManager = ((SimpleReloadableResourceManagerAccessor) resourceManager).idas_getFallbackResourceManager().get(fileID.getNamespace());
        List<IResourcePack> allResourcePacks = ((FallbackResourceManagerAccessor) namespaceResourceManager).idas_getPackList();
        ...
        for (IResourcePack resourcePack : allResourcePacks) {
            if (resourcePack.hasResource(ResourcePackType.SERVER_DATA, fileID)) {
                InputStream inputStream = ((FallbackResourceManagerAccessor) namespaceResourceManager).idas_callGetWrappedResource(fileID, resourcePack);
    ...
    private static void mergeIntoExistingPool(JigsawPattern feedingPool, JigsawPattern gluttonyPool) {
        ...
        List<JigsawPiece> elements = new ArrayList<>(((JigsawPatternAccessor) gluttonyPool).idas_getTemplates());
        List<Pair<JigsawPiece, Integer>> elementCounts = new ArrayList<>(((JigsawPatternAccessor) gluttonyPool).idas_getRawTemplates());

        elements.addAll(((JigsawPatternAccessor) feedingPool).idas_getTemplates());
        elementCounts.addAll(((JigsawPatternAccessor) feedingPool).idas_getRawTemplates());

        ((JigsawPatternAccessor) gluttonyPool).idas_setTemplates(elements);
        ((JigsawPatternAccessor) gluttonyPool).idas_setRawTemplates(elementCounts);
    }
