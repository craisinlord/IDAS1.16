...
public class SafeDecodingRegistryOps<T> extends WorldSettingsImport<T> {
    ...
    protected <E> DataResult<Pair<Supplier<E>, T>> decodeElement(T object, RegistryKey<? extends Registry<E>> registryKey, Codec<E> codec, boolean allowInlineDefinitions) {
        ...
        if (!optional.isPresent()) {
            return DataResult.error("(Integrated Dungeons and Structures SafeDecodingRegistryOps) Unknown registry: " + registryKey);
        } else {
            ...
            if (!dataResult.result().isPresent()) {
                return !allowInlineDefinitions ?
                        DataResult.error("(Integrated Dungeons and Structures SafeDecodingRegistryOps) Inline definitions not allowed here") :
                ...
                try {
                    ...
                    IDAS.LOGGER.error(
                            "\n Integrated Dungeons and Structures: Crash is about to occur because an entry in a datapack does not exist in a registry or failed to resolve an entry." +
    ...
    private <E> DataResult<Supplier<E>> readSupplier(RegistryKey<? extends Registry<E>> registryRegistryKey, MutableRegistry<E> mutableRegistry, Codec<E> codec, ResourceLocation elementId) {
        ...
        if (dataResult != null) {
            ...
            Supplier<E> supplier = Suppliers.memoize(() -> {
                ...
                if (object == null) {
                    throw new RuntimeException("(Integrated Dungeons and Structures SafeDecodingRegistryOps) Error during recursive registry parsing, element resolved too early: " + elementRegistryKey);
                } else {
                    return object;
                }
            });
