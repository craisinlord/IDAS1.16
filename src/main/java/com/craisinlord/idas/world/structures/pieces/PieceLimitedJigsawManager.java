...
import com.craisinlord.idas.mixin.structures.JigsawPatternAccessor;
import com.craisinlord.idas.mixin.structures.SingleJigsawPieceAccessor;
...
public class PieceLimitedJigsawManager {
    public static void assembleJigsawStructure(
        ...
        if(startPool == null || startPool.size() == 0){
            IDAS.LOGGER.warn("Integrated Dungeons and Structures: Empty or nonexistent start pool: {}  Crash is imminent", startPool == null ? structureID : (startPool.getName() + " - " + structureID));
        }
        ...
        for(int attempts = 0; runOnce || doesNotHaveAllRequiredPieces(components, requiredPieces); attempts++){
            if(attempts == 100){
                IDAS.LOGGER.error(
                        "\n-------------------------------------------------------------------\n"+
                        "Integrated Dungeons and Structures: Failed to create valid structure with all required pieces starting from this pool file: {}. Required pieces are: {}\n"+
                        ...
                        "  Otherwise, if the min and max height ranges aren't close and this message still appears, please report the issue to Integrated Dungeons and Structures's dev with latest.log file!\n\n",
        ...
        public void generatePiece(AbstractVillagePiece piece, MutableObject<BoxOctree> boxOctree, int minY, int depth, boolean doBoundaryAdjustments) {
            ...
            for (Template.BlockInfo jigsawBlock : pieceJigsawBlocks) {
                ...
                if (!(poolOptional.isPresent() && (poolOptional.get().size() != 0 || Objects.equals(jigsawBlockPool, JigsawPatternRegistry.EMPTY.location())))) {
                    IDAS.LOGGER.warn("Integrated Dungeons and Structures: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockPool, pieceBlueprint instanceof SingleJigsawPiece ? ((SingleJigsawPieceAccessor) pieceBlueprint).getTemplate().left().get() : "not a SingleJigsawPiece class");
                    continue;
                }
                ...
                if (!(fallbackOptional.isPresent() && (fallbackOptional.get().size() != 0 || Objects.equals(jigsawBlockFallback, JigsawPatternRegistry.EMPTY.location())))) {
                    IDAS.LOGGER.warn("Integrated Dungeons and Structures: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockFallback, pieceBlueprint instanceof SingleJigsawPiece ? ((SingleJigsawPieceAccessor) pieceBlueprint).getTemplate().left().get() : "not a SingleJigsawPiece class");
                    continue;
                }
                ...
                if (depth != this.maxDepth) {
                    JigsawPiece generatedPiece = this.processList(new ArrayList<>(((JigsawPatternAccessor)poolOptional.get()).idas_getRawTemplates()), doBoundaryAdjustments, jigsawBlock, jigsawBlockTargetPos, pieceMinY, jigsawBlockPos, octreeToUse, piece, depth, targetPieceBoundsTop);
                ...
                this.processList(new ArrayList<>(((JigsawPatternAccessor)fallbackOptional.get()).idas_getRawTemplates()), doBoundaryAdjustments, jigsawBlock, jigsawBlockTargetPos, pieceMinY, jigsawBlockPos, octreeToUse, piece, depth, targetPieceBoundsTop);
            }
        }
...
//                                    idas.LOGGER.warn(" Failed to spawn pieces. Parent: {} - General info: {} - Child: {} - General info: {} - Octree: {}", piece.getBoundingBox().toString(), piece.toString(), axisAlignedBBDeflated.toString(), candidatePiece.toString(), mutableObjectBoxOctree.toString());
//                                }
//                            }
